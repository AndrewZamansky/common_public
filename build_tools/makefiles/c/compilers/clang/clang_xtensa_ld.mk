...
LDFLAGS += --xtensa-core=$(CLANG_CORE)
...
ifdef CLANG_LINKER_SUPPORT_PACKAGE_DIR
    LDFLAGS += -mlsp=$(CLANG_LINKER_SUPPORT_PACKAGE_DIR)
endif
...
LINKER_TO_ASM_CMD = $(CLANG_ROOT_DIR)/bin/xt-objdump
LINKER_TO_ASM_CMD += --xtensa-core=$(CLANG_CORE)
LINKER_TO_ASM_CMD += --xtensa-system=$(CORE_CONFIG_DIR)
LINKER_TO_ASM_CMD +=-d -S $(LINKER_OUTPUT) > $(OUTPUT_ASM)

LINKER_TO_HEX_CMD = $(CLANG_ROOT_DIR)/bin/xt-objcopy
LINKER_TO_HEX_CMD += --xtensa-core=$(CLANG_CORE)
LINKER_TO_HEX_CMD += --xtensa-system=$(CORE_CONFIG_DIR)
LINKER_TO_HEX_CMD += -O ihex $(LINKER_OUTPUT) $(OUTPUT_HEX)

LINKER_TO_SREC_HEX_CMD = $(CLANG_ROOT_DIR)/bin/xt-objcopy
LINKER_TO_SREC_HEX_CMD += --xtensa-core=$(CLANG_CORE)
LINKER_TO_SREC_HEX_CMD += --xtensa-system=$(CORE_CONFIG_DIR)
LINKER_TO_SREC_HEX_CMD += -O srec $(LINKER_OUTPUT) $(OUTPUT_SREC_HEX)

LINKER_TO_BIN_CMD = $(CLANG_ROOT_DIR)/bin/xt-objcopy
LINKER_TO_BIN_CMD += --xtensa-core=$(CLANG_CORE)
LINKER_TO_BIN_CMD += --xtensa-system=$(CORE_CONFIG_DIR)
LINKER_TO_BIN_CMD += -O binary $(LINKER_OUTPUT) $(OUTPUT_BIN)

LINKER_TO_noDBG_CMD = $(CLANG_ROOT_DIR)/bin/xt-strip
LINKER_TO_noDBG_CMD += --xtensa-core=$(CLANG_CORE)
LINKER_TO_noDBG_CMD += --xtensa-system=$(CORE_CONFIG_DIR)
