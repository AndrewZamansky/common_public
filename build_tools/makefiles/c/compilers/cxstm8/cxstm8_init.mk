ifdef REDEFINE_CXSTM8_ROOT_DIR
    ...
    ifeq ("$(wildcard $(REDEFINE_CXSTM8_ROOT_DIR))","")
        $(info err: cxstm8 path $(REDEFINE_CXSTM8_ROOT_DIR) dont exists)
        $(info ---: to use default cxstm8 location remove/comment REDEFINE_CXSTM8_ROOT_DIR variable in  $(PARENT_OF_COMMON_PUBLIC_DIR)/workspace_config.mk )
        $(info ---: you can set customized cxstm8 path in REDEFINE_CXSTM8_ROOT_DIR variable in $(PARENT_OF_COMMON_PUBLIC_DIR)/workspace_config.mk )
        $(call exit,1)
    endif
...
#    TEST_CXSTM8_ROOT_DIR 	:=C:\Program\ Files\ (x86)\COSMIC\CXSTM8_EVAL
    ...
    ifeq ("$(wildcard $(TEST_CXSTM8_ROOT_DIR))","")
        $(info err: cxstm8 path $(TEST_CXSTM8_ROOT_DIR) dont exists )
