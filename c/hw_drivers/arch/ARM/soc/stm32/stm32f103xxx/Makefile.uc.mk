...
ifeq ($(sort $(CONFIG_STM32F10X)),y)
    ...
    ifeq ("$(wildcard $(BSP_STM32F10X_PATH))","")
        $(info !--- STM32F10X BSP path $(BSP_STM32F10X_PATH) does not exist)
        $(info !--- download STM32F10X BSP version $(STM32F103XXX_BSP_VERSION),)
        $(info !--- unpack it to $(BSP_STM32F10X_PATH))
        $(info !--- make sure Libraries and Project folders is located)
        $(info !--- in $(BSP_STM32F10X_PATH)/  after unpacking)
        $(error )
    endif
