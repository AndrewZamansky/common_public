...
void __attribute__((section(".critical_text"))) I2S_IRQHandler(void)
{
	...
	u32I2SIntFlag =
	   I2S_GET_INT_FLAG(I2S, (I2S_STATUS_RDMATIF_Msk | I2S_STATUS_RDMAEIF_Msk));
	...
	else
	{
		if(2 == num_of_bytes_in_word)
		{
			...
			{
				*pCurrTxBuf++ =
					(int16_t)(i2s_nuc505_dbg_gain * (float)(*pCurrRxBuf++));
				*pCurrTxBuf++ =
					(int16_t)(i2s_nuc505_dbg_gain * (float)(*pCurrRxBuf++));
			}
		}
		else if(4 == num_of_bytes_in_word)
		{
			...
			{
				*pCurrTxBuf++ =
					(int32_t)(i2s_nuc505_dbg_gain * (float)(*pCurrRxBuf++));
				*pCurrTxBuf++ =
					(int32_t)(i2s_nuc505_dbg_gain * (float)(*pCurrRxBuf++));
			}
		}
...
void demo_LineIn(void)
{
	...
	I2S_SET_INTERNAL_CODEC(I2S, 0x10, 0x08);  //Un-Mute the ADC Left ch volume
	I2S_SET_INTERNAL_CODEC(I2S, 0x11, 0x08);  //Un-Mute the ADC Right ch volume
	#else

	I2S_SET_INTERNAL_CODEC(I2S, 0x08, 0x00); // Un-Mute headphone of Left ch
	I2S_SET_INTERNAL_CODEC(I2S, 0x09, 0x00); // Un-Mute headphone of Right ch
	I2S_SET_INTERNAL_CODEC(I2S, 0x10, 0x07); //az Un-Mute the ADC Left ch volume
	I2S_SET_INTERNAL_CODEC(I2S, 0x11, 0x07); //az Un-Mute the ADC Right ch volum
	//I2S_SET_INTERNAL_CODEC(I2S, 0x10, 0x15);//Un-Mute the ADC Left ch volume
	//I2S_SET_INTERNAL_CODEC(I2S, 0x11, 0x05);//Un-Mute the ADC Right ch volume
	#endif
	//I2S_SET_INTERNAL_CODEC(I2S, 0x12, 0x00);//Un-Mute the ADC Side tone volume

	/* If Fs is changed, please Mute Headphone First and
	 * soft reset digital part after MCLK is stable. */
}
 ...
 void I2S_Init(struct I2S_nuc505_cfg_t *config_handle)
 {
	...
	uint32_t	sample_rate;
     ...
     CLK_EnableModuleClock(I2S_MODULE);
 		...
 		// APLL = 49152031Hz
     sample_rate = config_handle->sample_rate;
     if( (48000 == sample_rate) || (96000 == sample_rate))
     {
        /* I2S = 49152031Hz / (0+1) for 8k, 12k, 16k,
         *  24k, 32k, 48k, and 96k sampling rate
         */
     ...
     * Init I/O Multi-function
     ...
     /* Configure multi-function pins for I2S */
		...
		if (I2S_NUC505_API_MASTER_MODE == config_handle->clock_mode)
		{
			...
			I2S_Open(I2S, I2S_MODE_MASTER, config_handle->sample_rate,
					(num_of_bytes_in_word-1)<<I2S_CTL_WDWIDTH_Pos,
		...
		// Tx Start Address :
		I2S_SET_TXDMA_STADDR(I2S, (uint32_t) PcmTxBuff );
		// Tx Threshold Address :
		I2S_SET_TXDMA_THADDR(I2S, (uint32_t) (PcmTxThrBuff - 4) );
		// Tx End Address :
		I2S_SET_TXDMA_EADDR( I2S,
				(uint32_t) (PcmTxBuff + 2*buff_threshold_pos - 4));

		// Rx Start Address :
		I2S_SET_RXDMA_STADDR(I2S, (uint32_t) PcmRxBuff );
		// Rx Threshold Address :
		I2S_SET_RXDMA_THADDR(I2S, (uint32_t) (PcmRxThrBuff -4) );
		// Rx End Address :
		I2S_SET_RXDMA_EADDR( I2S,
				(uint32_t) (PcmRxBuff + 2*buff_threshold_pos - 4));
...
//		demo_MIC1();
		...
		I2S_CLR_INT_FLAG(I2S, I2S_STATUS_LZCIF_Msk|I2S_STATUS_RZCIF_Msk|
...
{
	struct I2S_nuc505_cfg_t *config_handle;
	struct I2S_API_set_params_t* set_params;
	...
	{
	case I2S_SET_PARAMS :
		set_params = (struct I2S_API_set_params_t*)aIoctl_param1;
		num_of_words_in_buffer_per_chenel =
					set_params->num_of_words_in_buffer_per_chenel;
		num_of_bytes_in_word = set_params->num_of_bytes_in_word;
		break;
	...
	case IOCTL_DEVICE_START :
		...
		num_of_words_in_buffer_per_chenel =
				config_handle->num_of_words_in_buffer_per_chenel;
