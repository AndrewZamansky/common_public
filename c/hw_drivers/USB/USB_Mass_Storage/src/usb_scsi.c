...
//  block_count = Mass_Block_Count[lun];
//  block_size = Mass_Block_Size[lun];

  block_count = USB_Mass_Storage_InstanceParams.BlockCount;
  block_size = USB_Mass_Storage_InstanceParams.BlockSize;

  ReadFormatCapacity_Data[4] = (uint8_t)(block_count >> 24);
  ReadFormatCapacity_Data[5] = (uint8_t)(block_count >> 16);
  ReadFormatCapacity_Data[6] = (uint8_t)(block_count >>  8);
  ReadFormatCapacity_Data[7] = (uint8_t)(block_count);

  ReadFormatCapacity_Data[9] = (uint8_t)(block_size >>  16);
  ReadFormatCapacity_Data[10] = (uint8_t)(block_size >>  8);
  ReadFormatCapacity_Data[11] = (uint8_t)(block_size);
...
//  }
  ...
  ReadCapacity10_Data[0] = (uint8_t)((block_count - 1) >> 24);
  ReadCapacity10_Data[1] = (uint8_t)((block_count - 1) >> 16);
  ReadCapacity10_Data[2] = (uint8_t)((block_count - 1) >>  8);
  ReadCapacity10_Data[3] = (uint8_t)(block_count - 1);

  ReadCapacity10_Data[4] = (uint8_t)(block_size >>  24);
  ReadCapacity10_Data[5] = (uint8_t)(block_size >>  16);
  ReadCapacity10_Data[6] = (uint8_t)(block_size >>  8);
  ReadCapacity10_Data[7] = (uint8_t)(block_size);
...
bool SCSI_Address_Management(uint8_t lun , uint8_t Cmd , uint32_t LBA , uint32_t BlockNbr)
{
  ...
  if ((LBA + BlockNbr) > USB_Mass_Storage_InstanceParams.BlockCount )
  {
    if (Cmd == SCSI_WRITE10)
    {
      Bot_Abort(BOTH_DIR);
  ...
  if (CBW.dDataLength != BlockNbr * USB_Mass_Storage_InstanceParams.BlockSize)
  {
    if (Cmd == SCSI_WRITE10)
    {
      Bot_Abort(BOTH_DIR);
