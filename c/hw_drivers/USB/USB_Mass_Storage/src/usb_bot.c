...
void Mass_Storage_In (void)
{
  switch (Bot_State)
  {
    ...
    case BOT_ERROR:
      Bot_State = BOT_IDLE;
    #ifndef STM32F10X_CL
      SetEPRxStatus(ENDP5, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
    #else
      if (GetEPRxStatus(EP5_OUT) == EP_RX_STALL)
      {
        SetEPRxStatus(EP5_OUT, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
      }
    #endif /* STM32F10X_CL */
      break;
    case BOT_DATA_IN:
...
//    	if//az test
//    	(
//    	(SCSI_TEST_UNIT_READY != CBW.CB[0])
//    		&& (SCSI_INQUIRY != CBW.CB[0])
//		)
//
//    	{
//    		CBW.CB[0]++;
...
{
  USB_SIL_Write(EP4_IN, Data_Pointer, Data_Len);
...
void Bot_Abort(uint8_t Direction)
{
  switch (Direction)
  {
    case DIR_IN :
      ...
      break;
    case DIR_OUT :
      SetEPRxStatus(ENDP5, EP_RX_STALL);
      break;
    case BOTH_DIR :
      SetEPTxStatus(ENDP4, EP_TX_STALL);
      SetEPRxStatus(ENDP5, EP_RX_STALL);
