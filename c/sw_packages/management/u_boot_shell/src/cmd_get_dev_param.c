		...
		cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	uint8_t DEV_SET_PARAM(
			char *dev_name_str , char *param_name_str , char *param_val_str)

	retVal= get_dev_ioctl((uint8_t*)argv[1] ,
			&dev_descriptor ,(uint8_t*)argv[2] , &pIoctlParam);
