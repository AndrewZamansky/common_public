...
ifeq ($(REDEFINED_ROOT_DIR),)
    $(info ---- directory for $(SEARCHED_TOOL) was not manually defined in $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE))
else
    $(info ---- trying to use $(SEARCHED_TOOL) located in directory defined by $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE))
    ...
    ifeq ($(wildcard $(REDEFINED_ROOT_DIR_FMT)),)
        $(info !--- directory $(REDEFINED_ROOT_DIR)   defined in $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE) was not found)
        $(info !--- redefine $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE)) 
        $(info !--- OR remove/comment $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE) if you want to use system  $(SEARCHED_TOOL)) 
        $(error )
    else
        #add escape char to spaces(needed for correct work of 'wildcard' function)
        ...
        ifeq ($(wildcard $(TEST_FILE_IN_SEARCHED_DIR_FMT)),)
            $(info !--- failed to use $(SEARCHED_TOOL) located in directory defined by $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE))
            $(info !--- $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE) = $(REDEFINED_ROOT_DIR))
            ...
            $(info !--- OR redefine $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE) to valid $(SEARCHED_TOOL) directory) 
            $(info !--- OR remove/comment $(WORKSPACE_ROOT_DIR)/workspace_config.mk/$(MANUALLY_DEFINED_DIR_VARIABLE) if you want to use system  $(SEARCHED_TOOL)) 
            $(error )
        else
...
ifeq ($(FOUND_ROOT_DIR),)
    ...
    ifeq ($(findstring WINDOWS,$(COMPILER_HOST_OS)),WINDOWS)
        TOOL_EXISTANCE_CHECK :=$(shell where $(SEARCHED_TOOL))
        ...
        FOUND_TOOL_CHECK_FMT :=$(subst *,$(SPACE),$(FOUND_TOOL_CHECK_FMT))
        FOUND_TOOL_CHECK :=$(filter %$(FOUND_TOOL_CHECK_FMT),$(TOOL_EXISTANCE_CHECK))
