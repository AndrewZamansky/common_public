...
ifeq ($(findstring menuconfig,$(MAKECMDGOALS)),)
    ...
    SHELL_OUT := $(shell $(MAKE) -f $(MAKEFILES_ROOT_DIR)/prebuild_routines/prebuild_check_unique_project_name.mk $(PASS_VARIABLES)  2>&1)
    ...
    include $(MAKEFILES_ROOT_DIR)/prebuild_routines/git_prebuild_routines.mk
...
ifeq ($(findstring menuconfig,$(MAKECMDGOALS)),) #dont enter if we are bulding .config file now
    ...
    ifdef CONFIG_ARM
        ifdef CONFIG_GCC
            include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_arm_init.mk
        else ifdef CONFIG_GPP
            include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_arm_init.mk
        else ifdef CONFIG_ARMCC
            include $(MAKEFILES_ROOT_DIR)/compilers/armcc/armcc_init.mk
        endif
    else ifdef CONFIG_PIC32
        include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_pic32_init.mk
    else ifdef CONFIG_AVR
        include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_avr_init.mk
    else ifdef CONFIG_STM8
        include $(MAKEFILES_ROOT_DIR)/compilers/cxstm8/cxstm8_init.mk
    else ifdef CONFIG_HEXAGON
        include $(MAKEFILES_ROOT_DIR)/compilers/clang/clang_hexagon_init.mk
    else ifdef CONFIG_ANDROID_NDK
        include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_android_init.mk
    else ifdef CONFIG_HOST
        ifeq ($(findstring WINDOWS,$(COMPILER_HOST_OS)),WINDOWS)
            ifdef CONFIG_MIN_GW_GCC
                include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_host_init.mk
            else ifdef CONFIG_MICROSOFT_COMPILER
                include $(MAKEFILES_ROOT_DIR)/compilers/microsoft_compiler/msvc_host_init.mk
            endif
        else
            include $(MAKEFILES_ROOT_DIR)/compilers/gcc/gcc_host_init.mk
        endif
